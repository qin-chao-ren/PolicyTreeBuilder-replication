...
输入优先：data/intermediate_outputs/v4_corpus_filtered.csv（若不存在则回退 v4_cluster_corpus_cleaned.csv）
...
输出：
  - data/intermediate_outputs/v4_corpus_calibrated.csv（新增列：doc_granularity, calibrated_level, calibration_confidence）
  - data/intermediate_outputs/v4_calibration_report.json
  - data/intermediate_outputs/v4_calibration_review.csv（低置信度样本，人工复核）
日志：data/intermediate_outputs/logs/step2_5.log
"""
...
def main() -> None:
    ...
    log_path = LOG_DIR_DEFAULT / "step2_5.log"

    corpus_path = Path(args.corpus)
    if not corpus_path.exists():
        fallback = OUT_DIR_DEFAULT / "v4_cluster_corpus_cleaned.csv"
        if fallback.exists():
    ...
    calibrated_path = out_dir / "v4_corpus_calibrated.csv"
    ...
    report_path = out_dir / "v4_calibration_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    log_write(log_path, f"[WRITE] {report_path}")

    if review_rows:
        review_path = out_dir / "v4_calibration_review.csv"
    ...
    if review_rows:
        print(f"[WRITE] {out_dir / 'v4_calibration_review.csv'}")
