...
DEFAULT_TREE = FINAL_TREE_DIR / "policy_tree_final.json"
DEFAULT_OUTDIR = FINAL_TREE_DIR
DEFAULT_MEMBERSHIP_DIR = INTERMEDIATE_DIR
DEFAULT_ADMIN_MAP = ADMINISTRATIVE_METADATA
...
def build_sample_records(
    ...
    for level in ["L4", "L3", "L2"]:
        path = membership_dir / f"tree_node_membership_{level}.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split final policy tree by province/city/admin name.")
    parser.add_argument("--tree", default=str(DEFAULT_TREE), help="Input policy_tree_final.json")
    parser.add_argument("--outdir", default=str(DEFAULT_OUTDIR), help="Output directory")
    parser.add_argument("--membership-dir", default=str(DEFAULT_MEMBERSHIP_DIR), help="Directory containing tree_node_membership_L4/L3/L2.csv")
    parser.add_argument("--admin-map", default=str(DEFAULT_ADMIN_MAP), help="Administrative metadata CSV")
    ...
    parser.add_argument("--output-stem", default="policy_tree", help="Output file stem for provincial and city trees.")
    parser.add_argument("--output-suffix", default="", help="Optional suffix before .json, e.g. _en_academic.")
