...
PROMPT_CLASSIFY = ROOT / "prompts" / "assign_parent_cluster.md"
...
def _load_assignments(outdir: Path) -> Dict[str, str]:
    path = outdir / "node_top_level_category_assignments.csv"
    if not path.exists():
        return {}
...
def _load_discards(outdir: Path) -> set[str]:
    path = outdir / "top_level_category_assignment_review.csv"
    if not path.exists():
        return set()
    try:
        df = pd.read_csv(path, dtype=str)
...
def _load_layer(outdir: Path, level: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    nodes_path = outdir / f"tree_nodes_{level}.csv"
    mem_path = outdir / f"tree_node_membership_{level}.csv"
...
def run_vertical_link(config_path: Path, from_level: str, to_level: str, l1_category: str | None):
    ...
    corpus_path = Path(paths.get("corpus", outdir / "policy_corpus_calibrated.csv"))
    emb_path = Path(paths.get("embeddings", outdir / "policy_corpus_embeddings.parquet"))
    pairs_path = Path(paths.get("pairs", outdir / "policy_similarity_rerank_edges.csv"))
    ...
    log_classify = LOG_DIR / "llm_parent_cluster_assignments.jsonl"
    ...
    # --- 1. 写入链接关系 (Append) ---
    links_path = outdir / f"tree_parent_links_{from_level}_to_{to_level}.csv"
    ...
    if new_parents:
        ...
        _append_with_dedup(new_nodes_df, outdir / f"tree_nodes_{to_level}.csv", ["node_id"])
        ...
        _append_with_dedup(new_mem_df, outdir / f"tree_node_membership_{to_level}.csv", ["node_id", "member_id"])
        ...
        _append_with_dedup(new_edges_df, outdir / f"cluster_similarity_edges_{to_level}.csv", ["node_a", "node_b"])
...
def main():
    parser = argparse.ArgumentParser(description="PolicyTreeBuilder final replication · Step3 Vertical Link (L4→L3 / L3→L2)")
