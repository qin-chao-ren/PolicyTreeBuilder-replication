#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render policy-tree JSON files as v5-style radial visualizations.
...
separately (e.g. ``v4_tree_final.json`` and
``v4_tree_final_en_academic_0509.json``).
"""
...
DEFAULT_BASE_DIR = PROJECT_ROOT / "data" / "final_tree"
...
def visual_path(path: Path, lang: str, image_format: str, tag: str = "radial_v5") -> Path:
    """Compute the output image path for *path* in *lang*.

    The input stem is normalised by stripping known language/version
    suffixes (``_en_academic_0509``, ``_en``) so that an English file
    such as ``v4_tree_final_en_academic_0509.json`` does not produce the
    awkward output ``v4_tree_final_en_academic_0509_en_radial_v5.jpg``.
    """
    stem = path.stem
    for suffix in ("_en_academic_0509", "_en"):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Render policy-tree JSON files as v5-style radial visualizations. "
    ...
    parser.add_argument("--output-tag", default="radial_v5",
                        help="Filename tag inserted before the language code "
                             "(default: radial_v5; use 'radial' to drop the version suffix)")
