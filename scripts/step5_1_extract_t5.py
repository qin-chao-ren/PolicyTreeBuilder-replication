...
# -*- coding: utf-8 -*-
"""
Step 5.1 · Extract T5 Action Units (With Traceability & De-duplication)
...
def main():
    ...
    parser.add_argument("--source", required=True, help="Raw RoundB source CSV")
    parser.add_argument("--output", required=True, help="Output T5 Units CSV")
    parser.add_argument("--env", default="configs/roundC_v4.env")
    ...
    for i, task in enumerate(tqdm(tasks, desc="Extracting")):
        ...
        try:
            # 调用 LLM
            resp, data = chat_json(
                system=step5_1_prompt["content"], # 直接传入 Prompt 字典
                user=user_content,
                model=llm_model,
                temperature=0.1,
                max_tokens=1000
