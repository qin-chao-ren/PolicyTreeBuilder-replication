...
"""
Step 1 · 标题清洗（v4 迁移实现，贴近 v2 行为）

改动点：
- 默认开启 LLM 清洗（--llm-clean yes），prompt 改为 prompts/step1_title_cleaned.md；
- ENV 对应：若设置 PRIMARY_LLM_MODEL/BASE_URL/API_KEY，则在调用前写入 OPENAI_BASE_URL/OPENAI_API_KEY 以兼容 v2 的 OpenAI 调用；
- 输出对应：主输出写入 data/intermediate_outputs/v4_cluster_corpus_cleaned.csv；同时保留 v2 风格的 roundC_cluster_corpus_cleaned.csv 作为兼容（可选）。
...
def main():
    ...
    ap.add_argument('--llm-clean-prompt', default=str(ROOT / 'prompts' / 'step1_title_cleaned.md'))
    ...
    raw_dump_path = out_dir / 'roundC_llm_raw.jsonl'
    ...
    # v4 主输出
    v4_out = out_dir / 'v4_cluster_corpus_cleaned.csv'
    cleaned_df.to_csv(v4_out, index=False, encoding='utf-8-sig')
    log_write(log_path, f"[WRITE] {v4_out} rows={len(cleaned_df)}")
    print(f"[WRITE] {v4_out}")

    # 兼容 v2 命名（可选）
    try:
        v2_out = out_dir / 'roundC_cluster_corpus_cleaned.csv'
        cleaned_df.to_csv(v2_out, index=False, encoding='utf-8-sig')
        log_write(log_path, f"[WRITE] {v2_out} rows={len(cleaned_df)}")
    except Exception:
        pass

    if pending:
        pd.DataFrame(pending).to_csv(out_dir / 'roundC_llm_pending.csv', index=False, encoding='utf-8-sig')
        log_write(log_path, f"[WRITE] {out_dir / 'roundC_llm_pending.csv'} rows={len(pending)}")
    ...
    (out_dir / 'v4_run_metadata.json').write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding='utf-8')
    log_write(log_path, f"[WRITE] {out_dir / 'v4_run_metadata.json'}")
