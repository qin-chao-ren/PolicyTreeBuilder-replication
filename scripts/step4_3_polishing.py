...
3. 最终产出：生成 v4_final_membership.csv，确保所有样本能找到最终归属。
"""
...
from common_utils import call_json, jaccard_overlap
...
PROMPT_POLISH = Path("prompts/step4_polish_merge.md")
...
class PolishingProcess:
    def __init__(self, env: Step4Env, tm: TreeManager, args):
        ...
        self.llm_log = env.log_dir / "llm_step4_polishing.jsonl"
    ...
    def _process_pair(self, a, b, case, parent_id):
        ...
        record = {
            "ts": int(time.time()), "step": "step4_3", "case": case,
    ...
    def _consolidate_all_traces(self) -> Dict[str, str]:
        ...
        # 1. 加载历史 Trace
        for step in ["4_1", "4_2"]:
            p = self.env.outdir / f"v4_trace_{step}.json"
            if p.exists():
                try:
    ...
    def _export_final_membership(self, trace_map: Dict[str, str]):
        """
        读取原始 CSV，应用 trace_map，生成 v4_final_membership.csv
        """
        output_rows = []
        
        for level in ["L4", "L3", "L2"]:
            path = self.env.outdir / f"v4_membership_{level}.csv"
            ...
            for _, row in df.iterrows():
                ...
                # 但 V4 逻辑尽量避免丢弃。如果真找不到，保留原 ID 或标记为 Unknown
        ...
        if output_rows:
            out_path = self.env.outdir / "v4_final_membership.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Round C v4 · Step4.3 Polishing")
