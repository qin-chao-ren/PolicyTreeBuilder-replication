...
# -*- coding: utf-8 -*-
"""
Step 3 · T2 -> L1 全局挂接（v4 · 三轮漏斗裁决版 · 配置增强）
...
def _bootstrap_l1_nodes(outdir: Path) -> pd.DataFrame:
    nodes_path = outdir / "v4_nodes_L1.csv"
    if nodes_path.exists():
        return pd.read_csv(nodes_path, dtype=str)
    def_path = outdir / "v4_l1_definition.json"
    if not def_path.exists():
        raise FileNotFoundError("Critical: v4_l1_definition.json not found.")
...
def _load_l3_context(outdir: Path) -> Dict[str, List[str]]:
    links_path = outdir / "v4_links_L3_to_L2.csv"
    nodes_path = outdir / "v4_nodes_L3.csv"
...
def run_link_t2_l1(config_path: Path, cli_hard_threshold: Optional[float]):
    ...
    l2_nodes = pd.read_csv(outdir / "v4_nodes_L2.csv", dtype=str)
    l2_mem = pd.read_csv(outdir / "v4_membership_L2.csv", dtype=str)
    ...
    print(f"[INFO] Processing {len(l2_nodes)} L2 nodes via 3-Round Strategy...")
    ...
    links_path = outdir / "v4_links_L2_to_L1.csv"
    ...
    if new_l1_nodes:
        ...
        safe_write_csv(l1_df, outdir / "v4_nodes_L1.csv")

    _log_line(LOG_DIR / "step3_link_t2_l1.log", f"[COMPLETE] Stats: {stats}")

def main():
    parser = argparse.ArgumentParser(description="Round C v4 · Step3 L2->L1 (3-Round Funnel)")
