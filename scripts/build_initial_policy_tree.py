...
# -*- coding: utf-8 -*-
"""
Step 3 · 构建全局粗树（public）
- 汇总 tree_nodes/tree_parent_links 生成 data/intermediate_outputs/policy_tree_initial.json。
- 可选 --emit-samples yes：在 L4 之下展开样本叶子（默认只到 L4）。
- 额外导出 policy_tree_initial_flat.csv、policy_tree_initial_levels.csv 供 Step 4 / 复核使用。
"""
...
def _load_nodes(outdir: Path, level: str) -> pd.DataFrame:
    path = outdir / f"tree_nodes_{level}.csv"
    if not path.exists():
...
def _load_links(outdir: Path, from_level: str, to_level: str) -> pd.DataFrame:
    path = outdir / f"tree_parent_links_{from_level}_to_{to_level}.csv"
    if not path.exists():
...
def build_tree(outdir: Path, corpus_path: Path | None, emit_samples: bool = False) -> Dict:
    ...
    mL4_path = outdir / "tree_node_membership_L4.csv"
...
def write_flat(tree: Dict, outdir: Path, emit_samples: bool):
    ...
    safe_write_csv(flat, outdir / "policy_tree_initial_flat.csv")
    ...
    safe_write_csv(pd.DataFrame(lev_rows), outdir / "policy_tree_initial_levels.csv")


def main():
    parser = argparse.ArgumentParser(description="PolicyTreeBuilder final replication · Step3 build coarse tree")
    ...
    corpus_path = Path(cfg.get("paths", {}).get("corpus", outdir / "policy_corpus_calibrated.csv"))
    emit_samples = args.emit_samples == "yes"

    tree = build_tree(outdir, corpus_path, emit_samples=emit_samples)
    safe_write_json(outdir / "policy_tree_initial.json", tree)
    write_flat(tree, outdir, emit_samples=emit_samples)
    log_path = ROOT / "data" / "intermediate_outputs" / "logs" / "build_initial_policy_tree.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    with log_path.open("a", encoding="utf-8") as fh:
        fh.write(f"[TREE] emit_samples={emit_samples} written\n")
    print(f"[WRITE] policy_tree_initial.json in {outdir}")
