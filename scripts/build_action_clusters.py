...
# -*- coding: utf-8 -*-
"""
Step 3 · 层内合并（public · 单模型极速版）

修改记录：
1. [单模型] 强制 Secondary Model = Primary Model，消除思路不一致，提升速度。
2. [极速] 只有一个成员的节点，直接复用原标题，不再调用 LLM。
3. [输入] 依赖修复后的 Top-level category (node_top_level_category_assignments.csv)。
"""
...
PROMPT_MERGE = ROOT / "prompts" / "decide_cluster_merges.md"
...
def run_layer_merge_for_partition(config_path: Path, level_L: str, l1_category: str | None):
    ...
    if l1_category:
        if not assign:
            raise ValueError("无法按 L1 切分：缺少 node_top_level_category_assignments.csv")
    ...
    nodes_path = outdir / f"tree_nodes_{level_L}.csv"
    mem_path = outdir / f"tree_node_membership_{level_L}.csv"
    ...
    edges_path = outdir / f"cluster_similarity_edges_{level_L}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="PolicyTreeBuilder final replication · Step3 Layer Merge (L4/L3/L2)")
