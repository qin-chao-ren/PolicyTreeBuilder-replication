...
from artifact_paths import FINAL_TREE_DIR, INTERMEDIATE_DIR

# 默认路径配置
DEFAULT_OUTDIR = INTERMEDIATE_DIR
CORPUS_FILE = DEFAULT_OUTDIR / "policy_corpus_cleaned.csv"
MEMBERSHIP_FILES = [
    DEFAULT_OUTDIR / "tree_node_membership_L4.csv",
    DEFAULT_OUTDIR / "tree_node_membership_L3.csv",
    DEFAULT_OUTDIR / "tree_node_membership_L2.csv"
]
OPS_LOG_FILE = DEFAULT_OUTDIR / "tree_edit_operations.jsonl"
FINAL_TREE_FILE = FINAL_TREE_DIR / "policy_tree_final.json"
OUTPUT_FILE = DEFAULT_OUTDIR / "node_lineage_trace.csv"
...
class LineageTracker:
    ...
    def _process_log_entry(self, entry: Dict):
        ...
        if op in ["merge", "cross_merge", "absorb_child"]:
            ...
            # === Case B: Finalization Structure Audit ===
            # 字段是: node_id (被合并者), merge_into (目标)
            if not loser and not winner:
                # Finalization 的 merge 逻辑
                if op == "merge":
                    loser = entry.get("node_id")
                    # 这里的 details 可能是 Finalization 记录 payload 的方式，也可能直接在顶层
...
def main():
    parser = argparse.ArgumentParser(description="Trace sample IDs from initial memberships to final tree nodes.")
    parser.add_argument("--corpus", default=str(CORPUS_FILE), help="Cleaned policy corpus CSV")
    parser.add_argument("--memberships", nargs="*", default=[str(p) for p in MEMBERSHIP_FILES], help="Membership CSV files")
    parser.add_argument("--operations", default=str(OPS_LOG_FILE), help="Tree edit operations JSONL")
    parser.add_argument("--tree", default=str(FINAL_TREE_FILE), help="Final policy tree JSON")
    parser.add_argument("--output", default=str(OUTPUT_FILE), help="Output lineage CSV")
    args = parser.parse_args()

    corpus_file = Path(args.corpus)
    membership_files = [Path(p) for p in args.memberships]
    ...
    for m_file in membership_files:
        ...
        with open(m_file, "r", encoding="utf-8") as f:
            ...
            # 根据 v3/public 惯例，L4 membership 通常是 node_id, sample_id
            for row in reader:
    ...
    tracker.replay_logs(operations_file)

    # 4. 加载最终树
    print("[4/5] Parsing Final Tree...")
    tracker.load_final_tree(tree_file)
