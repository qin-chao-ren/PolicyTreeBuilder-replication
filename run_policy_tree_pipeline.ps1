...
function Invoke-CheckedPython {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Python pipeline step failed with exit code $LASTEXITCODE."
  }
}

Write-Host "[1/14] Prepare policy corpus"
Invoke-CheckedPython scripts/prepare_policy_corpus.py `
...
Invoke-CheckedPython scripts/calibrate_policy_granularity.py `
  --corpus data/intermediate_outputs/policy_corpus_filtered.csv `
  --env configs/.env `
  --outdir data/intermediate_outputs

Write-Host "[5/14] Define top-level categories"
Invoke-CheckedPython scripts/define_top_level_categories.py `
...
Invoke-CheckedPython scripts/assign_top_level_categories.py `
...
Invoke-CheckedPython scripts/build_action_clusters.py --level L4 --config configs/tree_build_config.yaml

Write-Host "[8/14] Build L3 action clusters"
Invoke-CheckedPython scripts/build_action_clusters.py --level L3 --config configs/tree_build_config.yaml

Write-Host "[9/14] Build L2 action clusters"
Invoke-CheckedPython scripts/build_action_clusters.py --level L2 --config configs/tree_build_config.yaml

Write-Host "[10/14] Assign L4 clusters to L3 parents"
Invoke-CheckedPython scripts/assign_parent_clusters.py --from-level L4 --to-level L3 --config configs/tree_build_config.yaml

Write-Host "[11/14] Assign L3 clusters to L2 parents"
Invoke-CheckedPython scripts/assign_parent_clusters.py --from-level L3 --to-level L2 --config configs/tree_build_config.yaml

Write-Host "[12/14] Link L2 clusters to top-level categories"
Invoke-CheckedPython scripts/link_second_level_to_top_level.py --config configs/tree_build_config.yaml

Write-Host "[13/14] Build initial policy tree"
Invoke-CheckedPython scripts/build_initial_policy_tree.py --config configs/tree_build_config.yaml --emit-samples no

Write-Host "[14/14] Refine and finalize policy tree"
Invoke-CheckedPython scripts/collapse_redundant_hierarchy.py `
...
Invoke-CheckedPython scripts/balance_tree_structure.py `
...
Invoke-CheckedPython scripts/polish_tree_labels.py `
...
Invoke-CheckedPython scripts/finalize_policy_tree.py `
